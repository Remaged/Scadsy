...
class Mobile_api
{
   ...
   public function sendSms($mobile_number, $msg) {
    ...
    private function returnResult($response) {
		...
		if(isset($response->send_info->eventid)){
			$return->eventid = $response->send_info->eventid;
		}
		if(isset($response->call_result->error)){
			$return->error = $response->call_result->error;
		}
				
		//failed: no connection/response
        if ($response == NULL) {
            $return->pass = FALSE;
...
/* End of file mobile_api.php */
/* Location: ./modules/sms/models/mobile_api.php */